...
function messageContent(token, accountindex) {
	browser.tabs.query({ active: true, currentWindow: true, url: ["*://discord.com/*"] }, function (tabs) {
...
function renderGroup() {
	groups.forEach((group) => {
		...
		group_clone.querySelector("[total-account]").innerText = `${group.accounts.length} ${
...
function exportGroup(object, filename) {
	let blob = new Blob([JSON.stringify(object)], { type: "text/json;charset=utf-8" });
	browser.downloads.download({ url: URL.createObjectURL(blob), filename: `${filename}.txt` });
}
...
group_body.addEventListener("click", (a) => {
	if (a.target.classList.contains("groupholder")) {
		...
		a.currentgroup = groups.find(
			(group) => a.target.closest(".groupholder").querySelector("span").innerText == group.name,
		);
		a.currentgroup.accounts = JSON.parse(prompt("Paste the text file content."));
		save();
		render();

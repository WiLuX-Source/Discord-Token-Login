...
function render() {
	...
	document.querySelectorAll(
		"h4",
	)[1].innerText = `GROUP LIST — ${groups.length}`;
	renderTime();
}
...
confirmModalButtons[0].addEventListener("click", () => {
	if (groups.find((group) => group.name === footerinput.value))
...
group_body.addEventListener("click", (a) => {
	if (a.target.classList.contains("groupholder")) {
		...
		groups = groups.filter(
			(group) =>
				group.name !=
				a.target.closest(".groupholder").querySelector("span").innerText,
		);
		save();
		render();
		...
		reader.onloadend = () => {
			a.currentgroup.accounts = JSON.parse(reader.result);
			save();
			render();
...
account_body.addEventListener("click", (a) => {
	if (a.target.classList.contains("holderlogo")) {
		...
		messageContent(
			a.currentgroup.accounts[a.selectedaccount].token,
			a.selectedaccount,
		);
		...
		a.selectedaccount = a.target.closest(".accountholder").dataset.accountindex;
		a.currentgroup = groups.find((group) => selectedgroup == group.name);
		a.currentgroup.accounts = a.currentgroup.accounts.filter(
			(account, index) => index !== parseInt(a.selectedaccount),
		);
		save();
		render();
	}
